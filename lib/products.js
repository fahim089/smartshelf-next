...
export function buildImageUrl(filename) {
  ...
  // Strip any path prefix — DB stores bare filenames
  const file = filename.replace(/.*[/\\]uploads[/\\]/, '').replace(/.*[/\\]/, '')
  return `/uploads/${file}`
}
...
export async function listProducts(params = {}) {
  ...
  const [rows] = await db.execute(
    `SELECT p.id,p.name,p.sku,p.price,p.cost_price,p.unit,p.stock_quantity,p.low_stock_threshold,
            ...
            CASE WHEN p.stock_quantity=0 THEN 'out_of_stock' WHEN p.stock_quantity<=p.low_stock_threshold THEN 'low_stock' ELSE 'in_stock' END AS stock_status
     FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE ${where} ORDER BY p.name LIMIT ? OFFSET ?`,
