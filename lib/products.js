...
// Always return /api/uploads/filename so images are served dynamically
// This means new uploads show instantly without rebuild
export function buildImageUrl(filename) {
  if (!filename) return null
  if (filename.startsWith('http://') || filename.startsWith('https://')) return filename
  // Strip any path prefix — keep only bare filename
  const file = filename.replace(/.*[/\\]uploads[/\\]/i, '').replace(/.*[/\\]/g, '').trim()
  if (!file) return null
...
export async function listProducts(params = {}) {
  const { search, category_id, low_stock, page = 1, limit = 20 } = params
  const offset = (parseInt(page) - 1) * parseInt(limit)
  const conds  = ['p.is_active=1']
  const args   = []

  if (search) { conds.push('(p.name LIKE ? OR p.sku LIKE ?)'); const t = `%${search}%`; args.push(t, t) }
  if (category_id) { conds.push('p.category_id=?'); args.push(category_id) }
  if (low_stock === 'true') conds.push('p.stock_quantity <= p.low_stock_threshold')

  const where = conds.join(' AND ')
  const lim   = parseInt(limit)
  const off   = parseInt(offset)

  const [[{ total }]] = await db.execute(`SELECT COUNT(*) AS total FROM products p WHERE ${where}`, args)
  const [rows] = await db.execute(
    `SELECT p.id,p.name,p.sku,p.price,p.cost_price,p.unit,p.stock_quantity,p.low_stock_threshold,
     ...
     WHERE ${where} ORDER BY p.name LIMIT ${lim} OFFSET ${off}`,
    args
  )
  for (const r of rows) r.images = await fetchImages(r.id)
  return { data: rows, pagination: { total, page: parseInt(page), limit: lim, pages: Math.ceil(total / lim) } }
}
